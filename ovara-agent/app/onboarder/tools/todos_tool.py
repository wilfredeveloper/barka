...
class TodosTool:
    ...
    def _calculate_and_update_progress(self, client_id: str) -> Dict[str, Any]:
        ...
        # Update client progress with verification
        ...
        if update_result.modified_count == 0:
            logger.warning(f"Progress update for client {client_id} did not modify any documents. "
                         f"Matched: {update_result.matched_count}, Modified: {update_result.modified_count}")
        else:
            logger.info(f"Successfully updated progress for client {client_id} to {progress['overall']}%")
...
def update_todo_status_persistent(todo_id: str, status: str, collected_info: str, tool_context: ToolContext) -> dict:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Successfully updated client {client_id} progress to {progress['overall']}%")
        except Exception as progress_error:
            logger.error(f"Failed to update progress for client {client_id}: {str(progress_error)}")
            # Still return success for todo update, but log the progress update failure
            progress = {"overall": 0, "byPhase": {}, "totalTodos": 0, "completedTodos": 0}
